...
require(MCMCpack)
m1.mcmc <- MCMCregress(PRIfeel ~ rightide + econnat + female + ses + churchatt,data=mex)
summary(m1.mcmc)
@
...
<<label=Answer2>>=
m2.mcmc <- MCMCregress(PRIfeel ~ rightide + econnat + female + ses + churchatt + rightide:econnat,data=mex)
summary(m2.mcmc)
