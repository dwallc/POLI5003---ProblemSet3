...
Figure.2 displays the posterior distribution of the effect on ideology of the PRI. All included variables, except for chuirch attendence and Socio economic status have a positive effect on ideology. However, socioeconomic status has a negative effect on an individuals ideology towards the PRI and church attendance, because the confidence interval contains 0, has no effect.  
