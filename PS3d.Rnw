...
posterior1 <- MCMCregress(PRIfeel ~ rightide + econnat + female + ses + churchatt,data=mex)

@
I do not remember we run mcmcregression. Be honest, I am not sure what is ``default'' settings of MCMCpack! Can we just run rstan without any strings attached?
